    ...
    <body class="min-h-screen bg-[#FDFDFC] text-[#1b1b18] flex items-center justify-center p-6">
        <div class="w-full max-w-2xl rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
            <div class="flex items-center justify-between gap-4 pb-6 border-b border-gray-200">
                <div>
                    <h1 class="text-2xl font-semibold">Dashboard {{ ucfirst($user->role) }}</h1>
            ...
            <div class="mt-6 space-y-4 text-gray-700">
                @if ($user->role === 'admin')
                    <p>Ini adalah panel admin. Di sini Anda dapat melihat dan memproses semua pengaduan siswa.</p>
                @else
                ...
                <div class="rounded-2xl bg-gray-50 p-6 text-sm text-gray-700">
                    <p class="font-medium">Petunjuk singkat:</p>
                    <ul class="mt-3 list-disc space-y-2 pl-5">
                        <li>Admin dapat mengelola semua pengaduan.</li>
                        <li>Siswa dapat membuat dan melihat status pengaduan.</li>
                        <li>Untuk sementara, gunakan data akun yang sudah ada di tabel `users`.</li>
                    </ul>
                </div>
            </div>
        </div>
